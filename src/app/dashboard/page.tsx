          ...
          <div className="relative w-full max-w-2xl bg-stone-950 border border-amber-900/50 rounded-[2.5rem] overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.5)]">
              ...
              <div className="space-y-6">
                {preppedBooking ? (
                  <>
                    ...
                    <div className="mt-6">
                      <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-amber-500/60 mb-3">Confirmation Email</h4>
                      <input 
                        type="email" 
                        value={bookingEmail}
                        onChange={(e) => setBookingEmail(e.target.value)}
                        placeholder="Enter real email id for receipt" 
                        className="w-full bg-stone-900 border border-amber-900/50 rounded-xl p-3 text-sm text-stone-200 focus:border-amber-500/50 outline-none transition-colors" 
                      />
                    </div>
                  </>
                ) : (
            ...
            <div className="p-8 pt-0 flex gap-4">
              <button 
                  ...
                  try {
                    ...
                    if (navigator.geolocation) {
                        try {
                            ...
                            console.warn("Geolocation failed, falling back to IP/default.");
                        }
                    }
                    ...
                    const res = await fetch('/api/confirm-booking', {
                        ...
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({
                            booking_id: `${preppedBooking?.specialty?.toUpperCase() || 'GEN'}-REQ-${Date.now()}`,
                            preferred_time_window: preppedBooking?.recommended_window || 'Normal',
                            lat: userLat,
                            lon: userLon,
                            urgency: preppedBooking?.urgency || 'Medium',
                            email: bookingEmail,
                        })
                    });
                ...
                className={`flex-[2] py-4 rounded-2xl font-black uppercase tracking-[0.2em] text-sm transition-all duration-500 ${bookingStatus === 'booking' ? 'bg-stone-900 text-stone-700 cursor-not-allowed' : 'bg-gradient-to-r from-amber-600 to-amber-500 text-stone-950 hover:shadow-[0_0_30px_rgba(245,158,11,0.3)] shadow-lg'}`}
              >
              ...
              <div className="absolute inset-0 z-50 bg-stone-950/95 flex flex-col items-center justify-center p-8 text-center animate-fade-in">
                ...
                <div className="bg-stone-900 border border-amber-900/30 rounded-3xl p-6 w-full max-w-sm mb-8">
                  ...
                  {finalBookingData.report && (
                    <div className="bg-stone-950 p-4 rounded-xl border border-white/5 mb-4 text-xs text-stone-400 italic font-mono leading-relaxed">
                      {finalBookingData.report}
                    </div>
                  )}
